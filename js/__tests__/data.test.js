...
describe("loadAndRenderData", () => {
    ...
    test("loads and processes data correctly", async () => {
        ...
        expect(mockAppState.data.X2).toEqual([
            [0.5, 0.6],
            [0.7, 0.8],
        ]);
        expect(mockAppState.data.X3).toEqual([
            [0.9, 1.0],
            [1.1, 1.2],
        ]);
        ...
        expect(
            mockAppState.selection.selectedIndices.clear,
    ...
    test("calls UI functions in the correct order", async () => {
        ...
        const renderMetaCallOrder =
            mockRenderMetaInfo.mock.invocationCallOrder[0];
        const renderPlotsCallOrder =
            mockRenderAllPlots.mock.invocationCallOrder[0];
        const renderPaletteCallOrder =
            mockRenderColorPalette.mock.invocationCallOrder[0];
        const updateLabelsCallOrder =
            mockUpdateLabelCounts.mock.invocationCallOrder[0];
        const hideLoaderCallOrder =
            mockHidePlotLoader.mock.invocationCallOrder[0];
